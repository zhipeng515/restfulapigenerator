    ...
    var controllers = {
        getAll: {
            ...
            handler: getControllerHandlerFromOptions('getAll', options) || function (request, reply) {
                ...
                model.findAll(lastId, pageSize, filter, condition).then(
                    function (data) {
                        if (_.isEmpty(data)) {
                ...
                model.findByIdAndUpdate(request.params.id, request.payload, filter, condition).then(
                    function (data) {
                        if (_.isNull(data)) {
                            ...
                            reply({id: data.id}).location('/' + data.id); // HTTP 201;
                        }
                    },
                    function (error) {
                        if (error) {
...
//                schema: responses.remove.response
            ...
            handler: getControllerHandlerFromOptions('remove', options) || function (request, reply) {
                model.findByIdNoLean(request.params.id).then(
                    function (data) {
                        if (_.isNull(data)) {
                            reply(Boom.notFound('Cannot find ' + singularRouteName + ' with that id'));
...
//    var removeFilter = getNewFilter(defaultFilter, getControllerSchemaFilterFromOptions('remove', options));
    ...
    var controllersResponses = {
        ...
        remove: {
            ...
            condition: assert(getControllerSchemaConditionFromOptions('remove', options) == null,
                modelNameForAssert + ' controllers remove condition unsupport customized'),
            response: {}
            // filter: 'unsupport',
            // condition: 'unsupport',
            // response: 'unsupport'
...
function buildSchema(definitionObject, definitionJoin, options) {
    ...
    schema.statics.findAll = function (lastId, pageSize, filter, condition, callback) {
        ...
        if(_.isEmpty(populateSelect)){
            return this.find(defaultCondition).select(defaultFilter).limit(pageSize).sort({id: 'desc'}).lean().exec(callback);
        } else {
            return this.find(defaultCondition).select(defaultFilter).populate(populateSelect).limit(pageSize).sort({id: 'desc'}).lean().exec(callback);
        }
    };
